...
Learned how to destructure props to write cleaner and more readable code.
...
```jsx
function ListSeries(props) {
  const { name, id, rating, genre } = props.apidatas;
...
### Alternative: Direct Destructuring in Parameters

```jsx
function ListSeries({ apidatas }) {
  const { name, id, rating } = apidatas;
...
- Reduces repetitive `props.` references
- Makes dependencies clearer
- Easier to understand what data a component needs
